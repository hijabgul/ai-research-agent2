...
if "report_md" in st.session_state:
    ...
    # Render nicely (tables, headings, etc.)
    ...
    # -----------------------------------------------------------------------
    # PDF generation via fpdf2 (no system libs required)
    # -----------------------------------------------------------------------
    def _build_pdf(markdown_text: str) -> bytes:
        ...
        def _safe(text: str) -> str:
            """fpdf2's built-in fonts only handle Latin-1; strip anything else."""
            return (
                text.replace("\u2013", "-")
                .replace("\u2014", "-")
                .replace("\u2018", "'")
        ...
        for raw_line in markdown_text.splitlines():
            ...
            if line.startswith("# "):
                ...
                pdf.ln(2)
                pdf.multi_cell(0, 10, _safe(line[2:]))
                pdf.ln(2)
                ...
                # Render a table row as pipe-separated cells
                cells = [c.strip() for c in line.strip("|").split("|")]
                pdf.set_font("Helvetica", size=10)
                pdf.multi_cell(0, 6, _safe("   |   ".join(cells)))
                pdf.set_font("Helvetica", size=11)
    ...
    # ---- Download button (PDF, with .md fallback if anything goes wrong) --
    try:
